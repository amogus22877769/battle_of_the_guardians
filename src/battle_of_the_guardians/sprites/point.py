...
class Point(pygame.sprite.Sprite):
    def __init__(self, point_surface, relative_bar_size, relative_bar_thickness, window_size: tuple[int, int], relative_center_coordinates: tuple[float, float] = (0, 0), relative_width: float = 0) -> None:
        ...
        if self.relative_width == 0:
            self.image = pygame.transform.scale(self.real_image,
                                                (self.relative_bar_size[1] * self.window_size[1] - self.relative_bar_thickness[1] * 2 *
                                                 self.window_size[1],
                                                 self.relative_bar_size[1] * self.window_size[1] - self.relative_bar_thickness[1] * 2 *
                                                 self.window_size[1]))
        else:
    ...
    def copy(self):
        return Point(self.real_image, self.relative_bar_size, self.relative_bar_thickness, self.window_size, relative_width=self.relative_width)
