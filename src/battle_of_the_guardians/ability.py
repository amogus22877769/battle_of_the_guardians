...
class Ability:
    ...
    def cast(self, target_card: Card, cards: list[Card], target_opponent: Card, opponents: list[Card],
             flags: dict[str, set[Card], list[set[Card]]], energy_bar, strings, next_action=lambda : None) -> None:
        match self.kind:
            case 'fire_wave':
                ...
                for _ in range(3):
                    ...
                    opponent_index += 1
            case 'fire_breathe':
                def modifier(opp_index, opp):
                    def inner():
                        opp.current_health -= self.value
                        next_action() if not opp_index else next_action
                    return inner
                for opponent_index, opponent in enumerate(opponents):
                    ...
                    Animation([s],
                              ...
                              ).start(on_stop=modifier(opponent_index, opponent))
            case 'ice_wall':
